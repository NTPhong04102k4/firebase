...
const storeDataString = async (value: any, id: number) => {
  ...
  const multiGetItem=async()=>{
    let ArrayValue;
    try {
        ArrayValue=await AsyncStorage.multiGet(['name_key_index','name_key_index']);
    } catch (error) {
        console.log(error)
    }
...
export {storeDataObject,storeDataString,readData,readDataObject,getAllKeys,multiGetItem,removeFew,removeItem,clearAll}